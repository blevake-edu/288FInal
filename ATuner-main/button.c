...
void button_init() {
  ...
  // while ((SYSCTL_PRGPIO_R & 0x??) == 0) {};
	...
	// we need to clear bits 0-4 as inputs
...
uint8_t button_getButton() {
   ...
   /* int data = GPIO_PORTE_DATA_R;
    ...
    if (data & 1 == 0) { // Switch 1 is pushed
        return 1;
    }  if ((data >> 1) & 1 == 0) {
        return 2;
    }  if ((data >> 2) & 1 == 0) { // switch 3
        return 3;
    }  if ((data >> 3) & 1 == 0) {
        return 4;
    }*/
